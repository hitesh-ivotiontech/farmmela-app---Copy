...
import { View, StatusBar, Image, SafeAreaView, ImageBackground,Platform,Dimensions, Text,BackHandler,RefreshControl, FlatList, TouchableOpacity, TextInput, StyleSheet, Animated, } from 'react-native';
...
class Rent extends React.Component {
    ...
    constructor(props) {
        ...
        this.state = {
            index: 0,
            routes: [
                { key: 'first', title: global.languageData.Rent },
    ...
    getMyAds(catID,subCatID,subSubID) {
        ...
        console.log('getMyAds body',body)
    ...
    UNSAFE_componentWillReceiveProps(nextProps) {
        if (nextProps.GetAdvertisementSuccess) {
            ...
            isServerSuccess(data, this.props, (response) => {
                if (response.status == "true") {
                    ...
                    console.log('GetAdvertisementSuccess',response)
        ...
        if (nextProps.GetMyAdvertisementSuccess) {
            ...
            console.log('GetMyAdvertisementInfo',data)
            isServerSuccess(data, this.props, (response) => {
                if (response.status == "true") {
                    var myAds = this.state.myAds
                    myAds = myAds.concat(response.data)
                    console.log('GetMyAdvertisementInfo',response)
    ...
    searchFromList(catID,subcatID,subSubID, type) {
        ...
        if(type =='myads'){
            ...
            console.log('item is searchFromList',catID,subcatID,subSubID, type)
        }else{
            this.setState({
                data: [],
                filterData: [],
            ...
            console.log('item is',catID,subcatID,subSubID, type)
        }
        // type == 'myads' ?
    ...
    view = (type) => (
        global.user || type=='rent'?<View style={{ paddingHorizontal: 10, flex: 1 }}>
            <SearchAndFilter filterAction={() => this.filterModal.current.open()} onChangeText={(catID,subCatID) => { this.searchFromList(catID,subCatID, type) }} />
            <FlatList
                ...
                renderItem={({ item }) => (
                    <ListView props={this.props} item={item} type={type} from="Rent" refresh={() => {
                        this.componentWillMount()
                    }}
    ...
    render() {
        ...
        return (
            ...
            <SafeAreaView style={{ flex: 1, backgroundColor: "rgba(255,255,255,0.75)" }}>
                ...
                <Header name={global.languageData.Rent} props={this.props} showHome={true} />
                ...
                <FilterModal filterModal={this.filterModal} resetAction={this.reset} from='rent' onPress={(catID,subCatID,subSubID) => { this.searchFromList(catID,subCatID,subSubID, this.state.index == 0 ? 'rent' : 'myads') }} />
                <View style={{bottom:Platform.OS=='ios'?30:0}}>
                <AddButton props={this.props} data={"Rent"} refresh={this.componentWillMount.bind(this)} />
                </View>
